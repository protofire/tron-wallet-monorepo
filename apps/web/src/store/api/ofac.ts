  ...
  endpoints: (builder) => ({
    getIsSanctioned: builder.query<boolean, string>({
      async queryFn(address, { getState, dispatch }) {
        ...
        let chainsCache = apiSliceWithChainsConfig.endpoints.getChainsConfig.select(undefined)(state)
        ...
        if (!chainsCache.data) {
          ...
          chainsCache = apiSliceWithChainsConfig.endpoints.getChainsConfig.select(undefined)(updatedState)
        }
